...
public class PathFollowingPoints:MonoBehaviour
{
    [Tooltip("Vertices en el plano XZ (x → x, y → z en Unity)")]
    ...
    {
        if (numericsVertexes == null || numericsVertexes.Length == 0)
            return 0;
    ...
    {
        if (unityVertexes == null || unityVertexes.Length == 0)
        {
            numericsVertexes = new System.Numerics.Vector2[0];
            Debug.LogWarning("[PathFollowingPoints] unityVertexes es nulo o vacío.");
            return;
        }

        numericsVertexes = unityVertexes.ToNumericsArray();
        Debug.Log("[PathFollowingPoints] Camino inicializado con " + numericsVertexes.Length + " puntos.");
    }

    internal int GetClosestPoint(object position, int index)
    {
